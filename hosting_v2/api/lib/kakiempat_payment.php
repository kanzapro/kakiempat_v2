...
function kakiempat_payment_process_webhook(PDO $pdo, array $body): array
{
    ...
    if ($nominal === null) {
        v2ApiFail('invalid_nominal', 'Nominal pembayaran tidak valid.', 400);
    }
    ...
    $matchStmt = $pdo->prepare(
        "SELECT id, owner_user_id, sitter_user_id, total_price, payment_amount
         FROM kakiempa_v2_bookings
         WHERE status = 'awaitingPayment' AND payment_amount = ?
    ...
    try {
        ...
        if (is_array($row)) {
            ...
            $update = $pdo->prepare(
                "UPDATE kakiempa_v2_bookings
                 ...
                 WHERE id = ? AND status = 'awaitingPayment'",
            );
            $update->execute([$receivedAt, $namaVal, $bankVal, $totalPrice, $bookingId]);
        ...
        $awaiting = $pdo->query(
            "SELECT id, payment_amount FROM kakiempa_v2_bookings WHERE status = 'awaitingPayment' ORDER BY created_at ASC",
        );
        $awaitingRows = $awaiting ? $awaiting->fetchAll(PDO::FETCH_ASSOC) : [];
        if (count($awaitingRows) === 1) {
            ...
            if ((int) $only['payment_amount'] !== $nominal) {
                ...
                $mark->execute([$mismatchBookingId]);
            }
        }
