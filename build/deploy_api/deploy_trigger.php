...
$q=http_build_query(['repository_root'=>$c['repo_path']??'/home/kakiempa/repo_kakiempat','branch'=>$b]);
$ch=curl_init('https://127.0.0.1:2083/execute/VersionControl/update?'.$q);
curl_setopt_array($ch,[CURLOPT_RETURNTRANSFER=>1,CURLOPT_USERPWD=>($c['cpanel_user']??'kakiempa').':'.($c['cpanel_pass']??''),CURLOPT_TIMEOUT=>300,CURLOPT_SSL_VERIFYPEER=>0]);
$body=curl_exec($ch);$h=(int)curl_getinfo($ch,CURLINFO_HTTP_CODE);curl_close($ch);
$ok=$h===200&&is_string($body)&&str_contains($body,'"status":1');
http_response_code($ok?200:500);
echo json_encode(['ok'=>$ok,'http'=>$h]);
