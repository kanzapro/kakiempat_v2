...
function kakiempat_payment_v2_save_screenshot(?string $base64, ?string $mime): ?string
{
    ...
    $detected = match (true) {
        str_starts_with($raw, "\xFF\xD8\xFF") => 'jpg',
        str_starts_with($raw, "\x89PNG\r\n\x1a\n") => 'png',
        str_starts_with($raw, 'GIF87a'), str_starts_with($raw, 'GIF89a') => 'gif',
        str_starts_with($raw, 'RIFF') && substr($raw, 8, 4) === 'WEBP' => 'webp',
        default => null,
    };
    if ($detected === null) {
        v2ApiFail('invalid_screenshot', 'File harus berupa gambar (JPG, PNG, GIF, WebP).', 400);
    }
    $ext = $detected;
    if ($mime !== null) {
